...
The setup recipe uses Cargo for Rust tools and `uv sync --group dev` for project-managed Python tools. Semgrep, Ruff, Ty, actionlint, yamllint, and the changelog helper tests are pinned in `pyproject.toml` and invoked through `uv`.

## Line Length

Non-Rust tooling uses a 160-column policy: Ruff for Python support scripts, dprint for Markdown, and yamllint for YAML. Rust remains on the narrower `rustfmt` `max_width = 100` setting because wide Rust signatures, trait bounds, and method chains are harder to scan at 160 columns.
...
- `dprint.json` configures Markdown formatting with the repository's 160-column non-Rust line length. `CHANGELOG.md` is excluded because it is generated by `git-cliff`.
...
- `.yamllint` validates YAML with the repository's 160-column non-Rust line length.
