...
- `scripts/` — Python helpers for benchmark comparison and report promotion, notebook checks, shared changelog integration, dependency and tool-pin updates,
...
- Root configuration files (`Cargo.toml`, `rust-toolchain.toml`, `justfile`, `semgrep.yaml`, `dprint.json`, `rumdl.toml`, `typos.toml`,
