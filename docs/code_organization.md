...
- `docs/` — topic guides that support the public API documentation without duplicating the README generated region.
...
The crate root wires the public module layout together. It re-exports the core modules and defines the `prelude` for ergonomic user imports. The `//!` doc comment is the canonical source for both the docs.rs landing page and the README's API section, regenerated by `cargo-rdme` (see [`AGENTS.md` § Documentation generation](../AGENTS.md#documentation-generation)).
